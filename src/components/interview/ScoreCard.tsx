  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="flex justify-center">
        <div
          className="w-32 h-32 rounded-full border-4 flex items-center justify-center bg-surface-1 shadow-sm"
        ...
        {dims.map(({ key, label }) => (
          <div key={key}>
            ...
            <div className="w-full h-2 bg-surface-2 rounded-full overflow-hidden">
              <div className={`h-2 rounded-full transition-all duration-200 ${barColor(categories[key])}`} style={{ width: `${categories[key]}%` }} />
            </div>
          </div>
        ))}
      </div>
